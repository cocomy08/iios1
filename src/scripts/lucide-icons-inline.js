 ...
 * 用于替代 Lucide CDN，将常用图标内联为 SVG
 * 这样可以节省 ~1-2MB 内存，因为不需要加载整个 Lucide 库
 */
...
(function () {
    ...
    function createIcon(name, className) {
        ...
        if (!path) {
            console.warn(`[Lucide Inline] 未找到图标: ${name}`);
            return null;
        }
    ...
    function replaceLucideIcons() {
        ...
        if (replacedCount > 0) {
            console.log(`[Lucide Inline] ✅ 已替换 ${replacedCount} 个图标为内联 SVG`);
        }
    }
